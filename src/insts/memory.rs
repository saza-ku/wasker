...
use crate::inkwell_helpers::ptr_type;
...
pub fn memory_copy(
    ...
    let src_addr = resolve_pointer(
        ...
        ptr_type(
            environment.context,
            environment.inkwell_types.i32_type.into(),
        ),
        environment,
    )?;
...
pub fn generate_load<'a>(
    ...
    let dst_addr = resolve_pointer(
        offset,
        ptr_type(environment.context, load_type),
        environment,
    )?;
    // load value
...
pub fn generate_store<'a>(
    ...
    let dst_addr = resolve_pointer(
        offset,
        ptr_type(environment.context, store_type),
        environment,
    )?;
